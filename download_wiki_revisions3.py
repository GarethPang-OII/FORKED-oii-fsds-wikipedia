    ...
    if folder == "False":
        for item in os.listdir(path_to_subfolder):
            ...
            if os.path.isdir(item_path): # If item is a folder
                count += count_files(item_path,folder)  # Recursive call for subdirectories by repeating the function
            else:
        ...
        for item in os.listdir(path_to_subfolder):
            ...
            if os.path.isdir(item_path): # If the item is a folder
                count +=1 # Increment count for folders
                count += count_files(item_path, folder)
            else:
                continue
...
def main(page: str, limit: int, data_dir: Path, update:str, folder:str):
    """
    ...
    if update == "True":
        ...
        print(f"Identifying (but not downloading) {limit} revisions of {page} to {data_dir} \n")
        raw_revisions = download_page_w_revisions(page, limit=limit)
        validate_page(page, page_xml=raw_revisions)
...
if __name__ == "__main__":
    ...
    parser.add_argument("update", type=str, default="True", help="Download new XML or not")
    parser.add_argument("count_files", type=str, help="Counting revisions (represented by each file)")
    args = parser.parse_args()
    main(page=args.page, limit=args.limit, data_dir=DATA_DIR, update=args.update, folder=args.count_files)
